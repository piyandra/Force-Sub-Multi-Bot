...
@Component
public class DeleteDatabaseHandler implements CommandProcessor {
	private final SendingMessage sendingMessage;
	private final ForceSubService forceSubService;
	private final UserService userService;

	public DeleteDatabaseHandler(SendingMessage sendingMessage, ForceSubService forceSubService, UserService userService) {
	...
	private String mapAllForceSubToString(Long chatId) {
		Map<String, String> forceSubChannel = forceSubService.getForceSubChannel(userService.getUser(chatId).get(), ChannelType.DATABASE);
